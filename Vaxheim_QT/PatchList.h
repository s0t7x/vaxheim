#pragma once
#define WIN32_LEAN_AND_MEAN

#include <iostream>
#include <Windows.h>
#include <vector>
#include "Helper.h"
#include "Patch.h"

#include <qobject.h>

#define RAWSTR(x) #x

#define _ASM_RET "\xC3"
#define _ASM_RET_SIZE 1
#define _ASM_RET_TRUE "\xB0\x01\xC3"
#define _ASM_RET_TRUE_SIZE 3
#define _ASM_RET_FALSE "\xB0\x00\xC3"
#define _ASM_RET_FALSE_SIZE 3

class PatchList : public QObject {
	Q_OBJECT
public:
	std::vector<Patch*> internalPatchList;

	Patch * PlayerInGodMode;
	Patch * PlayerHaveStamina;
	Patch * PlayerIsDebugFlying;
	Patch * PlayerHaveRequirementsSub;
	Patch * PlayerHaveRequirementsMain;
	Patch * ShipIsWindControllActive;
	Patch * AttackGetLevelDamageFactor;
	Patch * InventoryIsTeleportable;
	Patch * MonsterAIIsSleeping;
	Patch * InventoryMoveInventoryToGrave;
	Patch * InventoryAddItem;
	Patch * InventoryGuiIsVisible;

	inline void scanAll() {
		for (auto p : internalPatchList) {
			p->scan();
		}
	}

	inline void patchAll() {
		for (auto p : internalPatchList) {
			p->patch();
		}
	}

	inline void unpatchAll() {
		for (auto p : internalPatchList) {
			p->unpatch();
		}
		PlayerHaveRequirementsMain->unpatch();
		InventoryGuiIsVisible->unpatch();
	}

	inline PatchList(HANDLE process) {
		PlayerInGodMode = new Patch{
		process,
		"\x48\x83\xEC\x00\x48\x89\x00\x00\x48\x8B\x00\x0F\xB6\x80\x51\x00\x00\x00\x48\x83\xC4\x00\xC3",
		"xxx?xx??xx?xxxx???xxx?x",
		(BYTE*)_ASM_RET_TRUE,
		_ASM_RET_TRUE_SIZE,
		RAWSTR(PlayerInGodMode)
		};
		internalPatchList.push_back(PlayerInGodMode);

		PlayerHaveStamina = new Patch{
			process,
			"",
			"xxx?xxx?xx??xx?xx???xx?????xx?xx?xx???xx????????xx?xxxx????xx?????xx?xx?xx???xxx????????xx?xxxx",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(PlayerHaveStamina)
		};
		internalPatchList.push_back(PlayerHaveStamina);

		PlayerIsDebugFlying = new Patch{
			process,
			"\x55\x48\x8B\xEC\x48\x83\xEC\x00\x48\x89\x00\x00\x48\x8B\x00\x48\x8B\x00\x00\x00\x00\x00\x48\x8B\x00\x83\x38\x00\x66\x90\x49\xBB\x00\x00\x00\x00\x00\x00\x00\x00\x41\xFF\xD3\x85\xC0\x74\x09\x0F\xB6\x00\x00",
			"xxxxxxx?xx??xx?xx?????xx?xx?xxxx????????xxxxxxxxx??",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(PlayerIsDebugFlying)
		};
		internalPatchList.push_back(PlayerIsDebugFlying);

		// Hammer
		PlayerHaveRequirementsSub = new Patch{
			process,
			"",
			"xxx?xxx????xx??xx??xx??xx??xx??xx??xx?xx?xx?xx??xx???xxx????????xx?xxxx????xxxxx",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(PlayerHaveRequirementsSub)
		};
		// Workbench / Inventory
		PlayerHaveRequirementsMain = new Patch{
			process,
			"\x55\x48\x8B\x00\x48\x83\xEC\x00\x48\x89\x00\x00\x48\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x4C\x8B\x00\x4C\x8B\x00\x49\x8B\x00\x49\x8B\x00\x85\xFF",
			"xxx?xxx?xx??xx??xx??xx??xx?xx?xx?xx?xx",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(PlayerHaveRequirementsMain)
		};
		internalPatchList.push_back(PlayerHaveRequirementsMain);

		// ShipIsWindControllActive = 0x2703D397880
		// "", "xxx?xxx?xx??xxxx??xx??xx??xx??xx??xx?xxx?xx?xx?xx?????xx????????xx?x????xx??xx?xx?xx?????xx"
		// "55 48 8B ? 48 83 EC ? 48 89 ? ? 33 C0 48 89 ? ? 48 89 ? ? 48 89 ? ? 48 8B ? ? 48 8B ? ? 48 8B ? 48 83 C2 ? 48 8B ? 83 38 ? 48 8D ? ? ? ? ? 49 BB ? ? ? ? ? ? ? ? 41 FF ? E9 ? ? ? ? 48 8B ? ? 48 8B ? 83 39 ? 48 8B ? ? ? ? ? 48 8B"
		ShipIsWindControllActive = new Patch{
			process,
			"",
			"xxx?xxx?xx??xxxx??xx??xx??xx??xx??xx?xxx?xx?xx?xx?????xx????????xx?x????xx??xx?xx?xx?????xx",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(ShipIsWindControllActive)
		};
		internalPatchList.push_back(ShipIsWindControllActive);

		// AttackGetLevelDamageFactor
		// Address of signature = 0x1F208A776B0
		// "\x48\x83\xEC\x00\x48\x89\x00\x00\xF3\x0F\x00\x00\x00\x00\x00\x00\xF3\x0F\x00\x00\x48\x8B\x00\x00\x48\x8B", "xxx?xx??xx??????xx??xx??xx"
		// "48 83 EC ? 48 89 ? ? F3 0F ? ? ? ? ? ? F3 0F ? ? 48 8B ? ? 48 8B"
		// float at +0x70
		AttackGetLevelDamageFactor = new Patch{
			process,
			"\x48\x83\xEC\x00\x48\x89\x00\x00\xF3\x0F\x00\x00\x00\x00\x00\x00\xF3\x0F\x00\x00\x48\x8B\x00\x00\x48\x8B",
			"xxx?xx??xx??????xx??xx??xx",
			(BYTE*)"\x00\x00\xc8\x42",
			sizeof(float),
			RAWSTR(AttackGetLevelDamageFactor),
			0x70
		};
		internalPatchList.push_back(AttackGetLevelDamageFactor);

		// InventoryIsTeleportable = 0x1EE1816D040
		// "\x55\x48\x8B\x00\x48\x83\xEC\x00\x48\x89\x00\x00\x33\xC0\x48\x89\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x48\x8B\x00\x00\x48\x8B\x00\x00\x48\x8B\x00\x48\x83\xC2\x00\x48\x8B\x00\x83\x38\x00\x48\x8D\x00\x00\x00\x00\x00\x49\xBB", "xxx?xxx?xx??xxxx??xx??xx??xx??xx??xx?xxx?xx?xx?xx?????xx"
		// "55 48 8B ? 48 83 EC ? 48 89 ? ? 33 C0 48 89 ? ? 48 89 ? ? 48 89 ? ? 48 8B ? ? 48 8B ? ? 48 8B ? 48 83 C2 ? 48 8B ? 83 38 ? 48 8D ? ? ? ? ? 49 BB"
		InventoryIsTeleportable = new Patch{
			process,
			"\x55\x48\x8B\x00\x48\x83\xEC\x00\x48\x89\x00\x00\x33\xC0\x48\x89\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x48\x8B\x00\x00\x48\x8B\x00\x00\x48\x8B\x00\x48\x83\xC2\x00\x48\x8B\x00\x83\x38\x00\x48\x8D\x00\x00\x00\x00\x00\x49\xBB",
			"xxx?xxx?xx??xxxx??xx??xx??xx??xx??xx?xxx?xx?xx?xx?????xx",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(InventoryIsTeleportable)
		};
		internalPatchList.push_back(InventoryIsTeleportable);

		// MonsterAIIsSleeping = 0x1DAEA16FE40
		// "", "xxx?xxx?xx??xx?xx??xx?xx?xx???xx????????xx?xxx?xxx?xx??xx?xx?xx??xx"
		// "55 48 8B ? 48 83 EC ? 48 89 ? ? 48 8B ? 48 8B ? ? 48 8B ? 83 38 ? 48 8D ? ? ? 49 BB ? ? ? ? ? ? ? ? 41 FF ? 85 C0 75 ? 33 C0 EB ? 48 8B ? ? 48 8B ? 83 39 ? 48 8B ? ? 44 0F"
		MonsterAIIsSleeping = new Patch{
			process,
			"",
			"xxx?xxx?xx??xx?xx??xx?xx?xx???xx????????xx?xxx?xxx?xx??xx?xx?xx??xx",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(MonsterAIIsSleeping)
		};
		internalPatchList.push_back(MonsterAIIsSleeping);

		// InventoryMoveInventoryToGrave = 0x1DA099A1F50
		// "\x55\x48\x8B\x00\x48\x81\xEC\x00\x00\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x48\x8B\x00\x48\x8B\x00\x33\xC0\x48\x89\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x48\x8B", "xxx?xxx????xx??xx??xx??xx??xx?xx?xxxx??xx??xx??xx"
		// "55 48 8B ? 48 81 EC ? ? ? ? 48 89 ? ? 48 89 ? ? 4C 89 ? ? 4C 89 ? ? 48 8B ? 48 8B ? 33 C0 48 89 ? ? 48 89 ? ? 48 89 ? ? 48 8B"
		InventoryMoveInventoryToGrave = new Patch{
			process,
			"\x55\x48\x8B\x00\x48\x81\xEC\x00\x00\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x48\x8B\x00\x48\x8B\x00\x33\xC0\x48\x89\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x48\x8B",
			"xxx?xxx????xx??xx??xx??xx??xx?xx?xxxx??xx??xx??xx",
			(BYTE*)_ASM_RET,
			_ASM_RET_SIZE,
			RAWSTR(InventoryMoveInventoryToGrave)
		};
		internalPatchList.push_back(InventoryMoveInventoryToGrave);

		// InventoryAddItem = 0x210A832A280
		// "\x55\x48\x8B\x00\x48\x81\xEC\x00\x00\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x4C\x8B\x00\x4C\x8B\x00\x4D\x8B\x00\x49\x8B\x00\x48\x8B\x00\x00\x49\x63\x00\x00\x44\x3B", "xxx?xxx????xx??xx??xx??xx??xx??xx??xx??xx?xx?xx?xx?xx??xx??xx"
		// "55 48 8B ? 48 81 EC ? ? ? ? 48 89 ? ? 48 89 ? ? 48 89 ? ? 4C 89 ? ? 4C 89 ? ? 4C 89 ? ? 4C 89 ? ? 4C 8B ? 4C 8B ? 4D 8B ? 49 8B ? 48 8B ? ? 49 63 ? ? 44 3B"
		// "remove" at +2ef
		InventoryAddItem = new Patch{
			process,
			"\x55\x48\x8B\x00\x48\x81\xEC\x00\x00\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x48\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x4C\x89\x00\x00\x4C\x8B\x00\x4C\x8B\x00\x4D\x8B\x00\x49\x8B\x00\x48\x8B\x00\x00\x49\x63\x00\x00\x44\x3B",
			"xxx?xxx????xx??xx??xx??xx??xx??xx??xx??xx?xx?xx?xx?xx??xx??xx",
			(BYTE*)"\x90\x90\x90",
			3,
			RAWSTR(InventoryAddItem),
			0x2ef
		};
		internalPatchList.push_back(InventoryAddItem);

		// InventoryGuiIsVisible = 0x74C243833D8DF170
		// "", "xxx?xxx?xx????????xx?xxx????????xx?xxx?xx????????xx?xx?????xx?xx??xx??xxxx??xx??x?xxxx??xxxxxxxxxx?x????xxxxxxxxxxx?xxx?xx"
		// "55 48 8B ? 48 83 EC ? 48 B8 ? ? ? ? ? ? ? ? 48 8B ? 90 49 BB ? ? ? ? ? ? ? ? 41 FF ? 85 C0 74 ? 48 B8 ? ? ? ? ? ? ? ? 48 8B ? 48 63 ? ? ? ? ? 83 F8 ? 40 0F ? ? 48 0F ? ? 85 C0 40 0F ? ? 48 0F ? ? EB ? 33 C0 48 8D ? ? 5D C3 00 00 00 00 00 00 01 04 ? 05 ? ? ? ? 00 00 00 00 00 00 00 00 55 48 8B ? 48 83 EC ? 48 B8"
		InventoryGuiIsVisible = new Patch{
			process,
			"",
			"xxx?xxx?xx????????xx?xxx????????xx?xxx?xx????????xx?xx?????xx?xx??xx??xxxx??xx??x?xxxx??xxxxxxxxxx?x????xxxxxxxxxxx?xxx?xx",
			(BYTE*)_ASM_RET_TRUE,
			_ASM_RET_TRUE_SIZE,
			RAWSTR(InventoryGuiIsVisible)
		};
	}
};

static PatchList * gPatchList = nullptr;